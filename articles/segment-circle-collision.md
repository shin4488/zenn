---
title: "ゲーム開発のための衝突判定 - 線分と円の当たり判定を三角関数で導こう"
emoji: "🎯"
type: "tech"
...
ゲームを作っていると、線分と円の当たり判定が欲しくなる場面がちょくちょくあります。レーザーが敵に当たったかどうかはまさにそうですし、振った剣が敵に当たったかどうかも同様に判定できます。ブロック崩しでボールがパドルで跳ね返るのも、ボールを円・パドルを線分と見立てれば同じ判定でできます。
...
もうひとつ、右側の図も見てください。θ が 90° を超えると先端が根元より後ろに回り込むので、横方向の距離 $L\cos\theta$ はマイナスになります。つまり **cosθ の符号を見れば、先端が「前」にあるか「後ろ」にあるかがわかります**。こちらは後半、円が線分の端からはみ出した位置にあるかを見分けるところで使います。
...
計算はベクトル(点 A から点 B へ向かう矢印)で進めます。まず下の図のように、2本のベクトル A と B を根元を揃えて並べて、なす角(矢印の間の角度)を θ とします。A を地面に、B を「θ だけ傾けて立てた長さ $|B|$ の棒」に見立てると、さっきの話の通り、B の横の長さは $|B|\cos\theta$、高さは $|B|\sin\theta$ です。
...
S と E の間にあるなら垂線はちゃんと線分の上に降りるので、d ≤ r がそのまま結論になります。外側にある場合は、線分の中で C に一番近い点は端点そのものなので、端点と C の距離を r と比べれば判定できます。
...
線分の長さが 0 だと `len(se)` が 0 になり、0 除算が起きます。TypeScript の 0 除算は例外にならず Infinity や NaN が返るだけなので、エラーは出ずに判定結果だけがおかしくなります。移動前後の座標から線分を作る使い方だと「その場から動かなかったフレーム」で発生するので、`se` と `sc` を作った直後でガードしておきます。
...
この判定は、高速な弾のすり抜け対策にも使えます。ゲームの当たり判定は1フレームごとにしか行われないため、弾が速いとこのフレームでは敵の手前、次のフレームでは敵の奥まで進んでしまい、どのフレームでも当たっていないことになります。つまりすり抜けが発生します。

弾の現在位置の点ではなく、前のフレームの位置から現在のフレームの位置までを線分として敵の円と判定すれば、飛び越えた区間ごと判定できるので、すり抜けは起きません。

![点判定と線分判定のすり抜け比較](/images/segment-circle-collision/fig7-tunneling.png)
