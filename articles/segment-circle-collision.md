...
長さ L の棒を、地面との角度が θ になるように斜めに立てたとします(下の図の左側)。このとき、根元から先端までの横方向の距離は $L \times \cos\theta$ になります。数式では掛け算の記号を省略して $L\cos\theta$ と書くので、この記事でも以降はそう書きます。同じように、高さは $L\sin\theta$ です。棒を寝かせるほど横に伸びて、立てるほど高くなります。つまり、
...
計算はベクトル(ある点から別の点へ向かう矢印)で進めます。まず下の図のように、2本のベクトル A と B を根元を揃えて並べて、なす角(矢印の間の角度)を θ とします。A を地面に、B を「θ だけ傾けて立てた長さ $|B|$ の棒」に見立てると、さっきの話の通り、B の横の長さは $|B|\cos\theta$、高さは $|B|\sin\theta$ です。
