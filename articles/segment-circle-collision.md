---
title: "ゲーム開発のための当たり判定 - 線分と円の衝突判定を三角関数で導こう"
emoji: "🎯"
type: "tech"
...
線分の長さが 0 だと `len(se)` が 0 になり、0 除算が起きます。TypeScript の 0 除算は例外にならず Infinity や NaN が返るだけなので、静かに壊れます。移動前後の座標から線分を作る使い方だと「その場から動かなかったフレーム」で発生するので、関数の先頭でガードしておきます。
